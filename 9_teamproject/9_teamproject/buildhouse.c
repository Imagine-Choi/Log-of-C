	...
	while (1) {
		...
		while (p != page) {
			...
			for (int k = 0; k < linecount; k++) {
				...
				if (build = '\0') {
					adele_house1(&right, 0);
				}
				else {
					adele_house1(&right, build);
				}
				count = 0;
				gotoxy(x, y + 1);
				...
				if (i == 1) {
					adele_house1(&right, build);//adele
				}
				if (i == 2) {
					adele_house1(&right, build);//maroon
				}
		...
		if (num == 3) {
			gotoxy(10, 16);
			printf("종료하시려면 q를 눌러주세요. 처음부터 재시작하려면 r를 눌러주세요:");
			scanf(" %c", &quit);
			if (quit == 'q')
				break;
			if (quit == 'r')
...
void adele_house1(int* right, int build) {
	if (build == 1) {
		*right = *right + 1;
	}
	switch (*right) {
