...
int longgame(int level) {
	while (1) {
		...
		srand(time(NULL));
		int re = 1, i, linecount = 0, page, p = 0, x = 2, y = 2, z = 5, w = 26, cnt = 0, k = 0, choice;
		int* num_count = 0, * comp_count = 0;
		int start, end, timelevel = 0, alltime = 0, startScore, finalScore, loseScore;
		...
		if (timelevel < alltime) {
			...
			finalScore = startScore - (alltime - (*pt)) * loseScore;
		}
		system("cls");
...
void textcolor(unsigned int text) {
...
void compare(char* texts, char* input, int z, int w, int timespent, float* maxsp, int* comp_count, int linecount, int start, int timelevel) {
	...
	//정타수
	correct = (strlen)(texts)-error;//-->이 경우 맞는 문자마다 correct를 늘려주는 것이 맞다
	...
	//gotoxy(79, 27);
	//printf("timespent:%d,correct:%d", timespent,correct);
	correct = 0;
	error = 0;
	accuracy = 0;
	speed = 0;
...
void pauselong(int* pt, int* resume, int* qe, int line) // 일시정지시간, 게임 재개 여부, 초기메뉴로 이동 여부, 출력할 줄
{
