...
int main(void)
{
	...
	while (1) //1번 반복문-(탈출조건:menu 가 4, restart가 2)
	{
		...
		case 1: //낱말 연습
		{
			...
			while (1) //2번 반복문 (탈출 mode가 3이거나 restart가 1이거나 함수호출 후)
			{
				do {
					game1_mode(); //모드 메뉴 출력
...
void waiting()
{
	...
	//입력창으로 이동(각 게임마다 입력창 위치가 달라서 함수 밖으로 뺄 예쩡)
	gotoxy(5, 27);
	printf("");
}
...
//일시정지 간단 버젼
void pause(int* pt, int* resume, int* qe, int line) // 일시정지시간, 게임 재개 여부, 초기메뉴로 이동 여부, 출력할 줄
{
...
void score_short(int T, int limit_time, int* short_totalscore) //걸린시간과 제한시간
{
	...
	if (T >= limit_time) //T가 제한시간이상일때
		score = 0;

	else //T가 제힌시간미만인 경우 10개의 구간으로 나누어 점수를 부여한다.
	{
	...
	*short_totalscore += score; //누적 점수에 현재 score를 추가해 누적 점수를 갱신해준다.
	//printf("\n이번 단어에 대한 점수는 %d 입니다.\ntoatl:%d", score, short_totalscore); // 나중에 입력창에 맞게 형식을 바꿀 예정입니다.
}
